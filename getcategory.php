...
extract($_POST);
if (isset($_POST) && !empty($category) && !is_null($category) && !empty($token)) {
...
//        $dtoken = JWT::decode($token, KEY, array("HS256"));
//        $dslno = $dtoken->slno;
            $dbrest = $db->query("select * from policies where category=:category", [":category" => $category]); //->get();
            if ($dbrest) {
...
//                $retres = ["data" => $retres];
                    ...
                    array_push($retres, ["code" => 201, "msg" => "Not Exists Any Policies"]);
                }
            } else {
